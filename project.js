    ...
    const render = () => {
        ...
        <th>phone no</th>
        <th>role</th>
        <th colspan=2>actions</th></thead><tbody>${this.data.map((user,index) => 
        {
        ...
        <td>${user.role}</td>
        <td>
        <button value='edit' id='edit' onClick='edit(${user.id})'class='btn btn-info'>edit</button>
        </td>
        <td>
        ...
        for (let i = 0; i < this.col.length-1; i++) {
             if('select'===this.col[i].type){const newcell = row.insertCell(i);
        ...
        newcell.innerHTML = `<td><button id=${row.id} class="btn btn-info" onclick=save(${row.id})>add</button></td><tr>`;
        ...
        for (let i = 0; i < this.col.length-1; i++) {
            const td = tab.getElementsByTagName("td")[i];
                ...
                if(this.col[i].type==='email'){                
                     if(!/^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]{2,5})\.([a-zA-Z]{2,5})$/.test(txtVal)){
                        obj=''  
                        alert('email invalid')
                        break;
                     }
            ...
            if (txtVal !== ''||this.col[i].required==='no') {
                    obj[this.col[i].colname] = txtVal.trim();      
                    obj.id=id      
                }           
                else {
                     obj = '';
            ...
            td.appendChild(ele);
                ...
                for (k = 0; k < this.role.length; k++) {
                    ele.innerHTML = ele.innerHTML +
                        '<option class="btn btn-primary dropdown-toggle" value="' + this.role[k] + '">' + this.role[k] + '</option>';
                }
                td.innerText = '';
                td.appendChild(ele);
           }
    ...
    tab.getElementsByTagName("td")[this.col.length-1].innerHTML = `<button type='sumbit' onclick='save(${id})'class="btn btn-success">save</button><button style='margin-left: 3px' onclick='render()'class="btn btn-warning">discard</button>`             
        }